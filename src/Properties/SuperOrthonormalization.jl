...
function split_merge(o1, o2)
    ...
    lambda_1 = reshape(sqrt.(D1), :, 1) .* u1'
    lambda_2 = u2 .* reshape(sqrt.(D2), 1, :)

    lambda_ = lambda_1 * lambda_2
    w1, S, w2 = svd(lambda_)
    
    x_inv = u1 .* reshape(1 ./ sqrt.(D1), 1, :)
    x_inv = x_inv * w1

    y_inv = w2' .* reshape(1 ./ sqrt.(D2), 1, :)
    y_inv = y_inv * u2'
    #@show S
    x_ = x_inv .* reshape(sqrt.(S), 1, :)
    y_ = y_inv .* reshape(sqrt.(S), :, 1)
