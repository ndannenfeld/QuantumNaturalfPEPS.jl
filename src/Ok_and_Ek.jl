...
function Ok_and_Ek(peps, ham_op; timer=TimerOutput(), Ok=nothing, kwargs...)
    ...
    S, pc, psi_S, env_top = @timeit timer "sampling" get_sample(peps) # draw a sample
    # TODO: Give env_top as argument to get_logψ_and_envs if it samlping_dim == contract_dim
    logψ, env_top, env_down = @timeit timer "vertical_envs" get_logψ_and_envs(peps, S) # compute the environments of the peps according to that sample
    ...
    E_loc = @timeit timer "energy" get_Ek(peps, ham_op, env_top, env_down, S, logψ, h_envs) # compute the local energy
