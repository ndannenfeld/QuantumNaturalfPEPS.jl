...
function Ok_and_Ek(peps, ham_op; timer=TimerOutput(), Ok=nothing, kwargs...)
    ...
    h_envs = get_all_horizontal_envs(peps, et, ed, S,) 
    E_loc = @timeit timer "energy" get_Ek(peps, ham_op, et, ed, S, logψ, h_envs) # compute the local energy
    grad = @timeit timer "log_gradients" get_Ok(peps, et, ed, S, h_envs, logψ; Ok) # compute the gradient
