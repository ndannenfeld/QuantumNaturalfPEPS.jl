...
function increase_bond_dim(o1::ITensor, o2::ITensor, new_bond_dim::Int; value=0)
    ...
    o2n = increase_bond_dim(o2, l, ln; value=0.)
...
"""
function increase_bond_dim!(peps::PEPS, new_bond_dim::Int; value=0)

    for i in 1:size(peps, 1), j in 1:size(peps, 2)
        if i < size(peps, 1)
            peps[i, j], peps[i+1, j] = increase_bond_dim(peps[i, j], peps[i+1,j], new_bond_dim; value)
        end
        if j < size(peps, 2)
            peps[i, j], peps[i, j+1] = increase_bond_dim(peps[i, j], peps[i,j+1], new_bond_dim; value)
        end
    end
